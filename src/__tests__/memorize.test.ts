...
describe('memorize middleware', () => {
  ...
  describe('remember stale-write guards', () => {
    ...
    it('does not write to cache when delete() is called while remember() is in-flight', async () => {
      const cache = memorize();
      const { promise: factoryPromise, resolve: resolveFactory } = deferred<string>();
      const factory = jest.fn().mockReturnValue(factoryPromise);
      const rememberPromise = cache.remember('key', factory);

      cache.delete('key');
      resolveFactory('stale');
      const result = await rememberPromise;

      expect(result).toBe('stale');
      expect(cache.getValue('key')).toBeUndefined();
    });

    it('calls factory again after delete() aborts the in-flight remember()', async () => {
      const cache = memorize();
      const { promise: factoryPromise, resolve: resolveFactory } = deferred<string>();
      const factory1 = jest.fn().mockReturnValue(factoryPromise);
      const factory2 = jest.fn().mockResolvedValue('fresh');
      const rememberPromise = cache.remember('key', factory1);

      cache.delete('key');
      resolveFactory('stale');
      await rememberPromise;

      await cache.remember('key', factory2);
      expect(factory2).toHaveBeenCalledTimes(1);
      expect(cache.getValue('key')).toBe('fresh');
    });

    it('does not write to cache when deleteMatching() is called while remember() is in-flight', async () => {
      const cache = memorize();
      const { promise: factoryPromise, resolve: resolveFactory } = deferred<string>();
      const factory = jest.fn().mockReturnValue(factoryPromise);
      const rememberPromise = cache.remember('user:1', factory);

      cache.deleteMatching('user:*');
      resolveFactory('stale');
      await rememberPromise;

      expect(cache.getValue('user:1')).toBeUndefined();
    });

    it('does not write to cache when clear() is called while remember() is in-flight', async () => {
      const cache = memorize();
      const { promise: factoryPromise, resolve: resolveFactory } = deferred<string>();
      const factory = jest.fn().mockReturnValue(factoryPromise);
      const rememberPromise = cache.remember('key', factory);
    ...
    it('does not overwrite a newer set() when remember() is in-flight', async () => {
      const cache = memorize();
      const { promise: factoryPromise, resolve: resolveFactory } = deferred<string>();
      const factory = jest.fn().mockReturnValue(factoryPromise);
      const rememberPromise = cache.remember('key', factory);
      ...
      cache.set('key', 'newer');
      resolveFactory('from-factory');
      await rememberPromise;

      expect(cache.getValue('key')).toBe('newer');
    });
  });
